...
const transformer = module.exports = function(cb) {
  ...
  fs.readFile(__dirname + '/../data/non-palette-bitmap.bmp', (err, data) => {
    ...
    writeAndExportBuffer(headers, data, transPixels);
    if(cb) {cb(buffer);}
  });
}
...
const transformPixelsArray = function(pixels, transPixels) {
...
const writeAndExportBuffer = function(headers, data, transPixels) {
